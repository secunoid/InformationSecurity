...
=== Centralized
All web platofors are in use today are deployed using the centralized model.  For instance, _Facebook_ has all user data in their data centres.  Centralization introduces following risks:

* Mis-use of data
* Honeypot, where there is so much data in one location that it becomes a target for hackers to gain access to by whatever means possible.
* Censorship
* Inappropriate use and access to user data.
* Monitoring and disciplining of citizens
...
<<<
== Privacy Impacts
Centralization has very damaging impacts 
